...
async function addNewUser(event) {
    ...
    await fetch(url, method).then(() => {
        form_new.reset();
        getAdminPage();
        activeTabContent('all-users-tab');
        let activateTab = document.getElementById('all-users-tab');
        activateTab.classList.add('active');
        let deactivateTab = document.getElementById('new-user-tab');
...
async function editModalData(id) {
    ...
    const urlDataEd = 'http://localhost:8080/api/admin/users/' + id;
...
async function editUser() {
    let urlEdit = 'http://localhost:8080/api/admin/users/' + id_ed.value;
    let listOfRole = [];
...
async function deleteUser() {
    let urlDel = 'http://localhost:8080/api/admin/users/' + id_del.value;
    let method = {
        method: 'DELETE',
        headers: {
            "Content-Type": "application/json"
